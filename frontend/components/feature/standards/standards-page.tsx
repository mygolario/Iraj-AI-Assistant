...
export function StandardsPage() {
  ...
  const handleIndex = async (uploaded: File[]) => {
    ...
    try {
      ...
      toast.success(`Indexed ${res.indexed.length} file(s)`, {
        description: `${res.total_records} total passages`,
      });
      const s = await api.rag.state();
      setRecords(s.records);
      setFiles(s.files_list);
    } catch (e) {
      toast.error("Indexing failed", { description: (e as Error).message });
    } finally {
      setIndexing(false);
    }
  };
  ...
  const handleCompile = async () => {
    if (!effectiveGrade.trim()) {
      toast.error("Select or enter a grade first");
      return;
    }
    setCompiling(true);
    setDatasheet(null);
    try {
      ...
      toast.error("Datasheet compilation failed", { description: (e as Error).message });
    } finally {
      setCompiling(false);
    }
  };
    ...
    <div className="flex flex-col gap-6">
      <div className="grid gap-5 lg:grid-cols-[1fr_1.4fr]">
        <div className="flex flex-col gap-4">
          <FileDropzone
            ...
            multiple
            onFiles={handleIndex}
            label="Upload standards (PDF or TXT)"
            hint="DIN, ASTM, JIS, SAE, GB/T, BS — index multiple files"
          />
          <div className="glass rounded-2xl p-4">
            ...
            {files.length > 0 ? (
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                No standards indexed yet. Upload files above to enable semantic search.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="glass gradient-border rounded-2xl p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="font-display text-sm font-semibold">Semantic Query</h3>
          </div>
          <form onSubmit={handleSearch} className="relative">
            ...
            <input
              ...
              placeholder="e.g. DIN 488 yield strength, ASTM A615 chemical composition"
            ...
            {!searching && results && results.length === 0 && (
              <div className="flex flex-col items-center gap-2 rounded-xl bg-white/[0.03] py-8 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  No matches in the indexed standards.
                </p>
              </div>
            )}
            {!searching &&
      ...
      <div className="glass gradient-border rounded-2xl p-5">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h3 className="font-display text-sm font-semibold">Technical Datasheet Generator</h3>
          <span className="ml-auto text-[10px] text-muted-foreground">
            Compiled exclusively from your RAG index — no fabricated specs
          </span>
        </div>
        <div className="grid gap-3 sm:grid-cols-3">
          <label className="flex flex-col gap-1.5">
            <span className="text-xs font-medium text-muted-foreground">Rebar Grade</span>
            <select
              value={grade}
          ...
          {grade === "Custom" && (
            <label className="flex flex-col gap-1.5">
              <span className="text-xs font-medium text-muted-foreground">Custom Grade</span>
              <input
                value={customGrade}
                onChange={(e) => setCustomGrade(e.target.value)}
                placeholder="e.g. GB/T 1499.2 HRB400"
                className="h-10 rounded-xl border border-white/10 bg-white/[0.03] px-3 text-sm text-foreground outline-none focus:border-accent/60"
              />
            </label>
          )}
          <label className="flex flex-col gap-1.5 sm:col-span-1">
            <span className="text-xs font-medium text-muted-foreground">Manufacturer / Client</span>
            <input
              value={company}
          ...
          >
            {datasheet.available ? (
              <>
                <div className="mb-3 flex items-center justify-between">
                  <span className="flex items-center gap-2 text-sm font-semibold text-success">
                    ...
                    Datasheet compiled from {datasheet.sources.length} source(s)
                  </span>
                  <button
                ...
                <div className="overflow-hidden rounded-lg border border-white/10">
                  <table className="w-full text-left text-xs">
                    <tbody>
                      {[
                        ["Grade", datasheet.grade],
                        ["Min Yield Strength", datasheet.yield_strength ?? "N/A"],
                        ["Min Tensile Strength", datasheet.tensile_strength ?? "N/A"],
                        ["Sizes Available", datasheet.size_range ?? "N/A"],
                        ["Chemical Composition", datasheet.chemical_composition ?? "N/A"],
                      ].map(([k, v]) => (
              ...
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                Specification not available for &ldquo;{datasheet.grade}&rdquo;.
                Upload the relevant standard to enable datasheet generation.
              </div>
            )}
          </motion.div>
        )}
